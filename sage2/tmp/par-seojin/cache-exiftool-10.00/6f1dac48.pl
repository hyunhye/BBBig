...
# add arguments embedded in filename
if ($0 =~ /\(([^\\\/]+)\)(.exe|.pl)?$/i) {
    my $argstr = $1;
...
sub FilenameSPrintf($;$@)
{
    ...
    return $fmt unless $fmt =~ /%[-+]?\d*[.:]?\d*[lu]?[dfetgs]/;
    ...
    $part{e} = $part{e} ? substr($part{e}, 1) : '';
    @part{qw(t g s)} = @extra;
    my ($filename, $pos) = ('', 0);
    while ($fmt =~ /(%([-+]?)(\d*)([.:]?)(\d*)([lu]?)([dfetgs]))/g) {
...
sub OpenOutputFile($;@)
{
    ...
    if ($textOut) {
        ...
        if ($textOut =~ /%[-+]?\d*[.:]?\d*[lun]?[dfetgscC]/ or defined $tagOut) {
  ...
  Option Details
   ...
   Processing control
    ...
    -z (-zip)
         ...
         format. (eg. The PNG format supports compressed text.) This option
...
__HELP__
    ...
    if ($dir) {
        ...
        $dir = $ENV{TMP} || $ENV{TEMP} || '/tmp';
    }
    $tmpFile = "$dir/exiftool_doc.txt";
    # try backslashes first if it seems we may be running in cmd.exe
    $tmpFile =~ tr/\//\\/ if $ENV{ComSpec} or $tmpFile =~ /\\/;
    unless (open(TEMP, ">$tmpFile")) {
        $tmpFile =~ tr/\/\\/\\\//;  # try the other slash
        unless (open(TEMP, ">$tmpFile")) {
    ...
    print TEMP $help;
    close TEMP;
    system(qq{more < "$tmpFile"}) and warn "Error running more $tmpFile\n";
    unlink $tmpFile;
    undef $tmpFile;
}
